                        ...
                        uint8_t *error_out)
{
    ...
    /*
     * CORREÇÃO (confirmada contra o cdrom.c real do Free60 XeLL):
     * BYTE_CNT_L/BYTE_CNT_H esperam a contagem em WORDS (16 bits), não em
     * bytes. O XeLL faz "data_len /= 2" antes de gravar nesses registradores;
     * aqui usamos 'words' (que já é data_len/2, calculado acima) em vez do
     * data_len cru. Escrever o valor em bytes sem dividir fazia o
     * controlador receber o dobro da contagem esperada em qualquer comando
     * com transferência de dados (REQUEST SENSE, READ TOC, READ CD) — o
     * TEST UNIT READY não era afetado porque data_len=0 é igual dividido
     * ou não.
     */
    out8(REG_BCNTL, (uint8_t)(words & 0xFF));
    out8(REG_BCNTH, (uint8_t)(words >> 8));
    out8(REG_FEATURES, 0);
